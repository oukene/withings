...
from homeassistant.const import CONF_WEBHOOK_ID, STATE_UNKNOWN, STATE_UNAVAILABLE
...
from . import const
from .const import Measurement, DOMAIN
...
class ConfigEntryWithingsApi(AbstractWithingsApi):
    ...
    def _request(
        ...
        _LOGGER.debug("response : " + str(response.json()))
...
class BaseWithingsSensor(Entity):
    ...
    def __init__(
        ...
        #     f"Withings {description.measurement.value}"
        # )
    ...
    def translation_key(self) -> str | None:
        _LOGGER.debug("translation key : " + str(self.entity_description.key))
    ...
    def available(self) -> bool:
        ...
        if self.entity_description.update_type == UpdateType.POLL:
            _LOGGER.debug(
                "availiable : %d", self._data_manager.poll_data_update_coordinator.last_update_success)
...
async def async_get_data_manager(
    ...
    _LOGGER.debug("config_entry_data : " + str(config_entry_data))
